//
//  EyeGazeTrackingInterpolator.swift
//  Eyes Tracking
//
...
class EyeGazeTrackingInterpolator {

    private var previousGaze: UIEyeGaze?
    final var event: UIEyeGazeEvent?
    ...
    final func update(withFrame frame: ARFrame, correctionAmount: CGSize, scale: CGFloat) {
        ...
        if faceAnchor.isTracked {
            ...
            // Generate eye gaze event and invoke event callback methods
            var allGazes = Set<UIEyeGaze>()
            let curGaze: UIEyeGaze
            if let lastGaze = previousGaze {
                curGaze = UIEyeGaze(curPosition: cursorPosNDC, prevPosition: lastGaze.location(in: window), view: view, win: window)
            } else {
                curGaze = UIEyeGaze(position: cursorPosNDC, view: view, win: window)
            }

            allGazes.insert(curGaze)
            previousGaze = curGaze
            event = UIEyeGazeEvent(allGazes: allGazes)
    ...
    /**
     return eye gaze projection in 2D NDC coordinate system
    ...
    private func updateGazeNDCLocationByARFaceAnchor(frame: ARFrame, isBlinking: Bool, correctionAmount: CGSize, scale: CGFloat) -> CGPoint {
        ...
        let o_eyeCenter = simd_float4(0, 0, 0, 1)
        let o_eyeLookAtDir  = simd_float4(0, 0, 1, 0)

        let tranfMtx = worldTransMtx
        let c_eyeCenter = tranfMtx * o_eyeCenter
        let c_lookAtDir  = tranfMtx * o_eyeLookAtDir
        let t = (0.0 - c_eyeCenter[2]) / c_lookAtDir[2]
        let hitPos = c_eyeCenter + c_lookAtDir * t
...
final class PIDControlledTrackingInterpolator: EyeGazeTrackingInterpolator {
...
final class LowPassTrackingInterpolator: EyeGazeTrackingInterpolator {
