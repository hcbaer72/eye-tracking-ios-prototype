...
extension Notification.Name {
    ...
    static let eyeTrackingDisabled = Notification.Name("eyeTrackingDisabled")
}
...
class UIEyeGazeViewController: UIViewController, ARSessionDelegate, ARSCNViewDelegate {
 
    private(set) var sceneview: ARSCNView?
    private lazy var receivingWindow: EyeGazeWindow? = {
        for window in UIApplication.shared.windows {
            if let result = window as? EyeGazeWindow {
                return result
            }
        }
        return nil
    ...
    let debugInterpolator = EyeGazeTrackingInterpolator()
    lazy var trackingInterpolators: [EyeGazeTrackingInterpolator] = [pidInterpolator, debugInterpolator]
    ...
    func renderer(_ renderer: SCNSceneRenderer, didUpdate node: SCNNode, for anchor: ARAnchor) {
        ...
        guard let eyeNode = eyeNode, let sceneView = sceneview else { return }
        let vector = eyeNode.convertPosition(SCNVector3Zero, to: sceneView.pointOfView)
