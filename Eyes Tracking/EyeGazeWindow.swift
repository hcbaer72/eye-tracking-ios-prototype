...
class EyeGazeWindow: UIWindow {
    ...
    private var lastGaze: UIEyeGaze?
    ...
    private func commonInit() {
        ...
        AppConfig.$isEyeTrackingEnabled.sink { [weak self] isEnabled in
    ...
    func presentEyeTrackingErrorToastIfNeeded() {
        guard !UIApplication.shared.isGazeTrackingActive, AppConfig.isEyeTrackingEnabled, !trackingDisabledByTouch else {
            return
        }
        let title = String(localized: "gaze_tracking.error.excessive_eye_distance.title")
        ...
        func schedule(duration: TimeInterval = self.touchGazeDisableDuration) {
            DispatchQueue.main.asyncAfter(deadline: .now() + duration) {
                ...
                if passed >= self.touchGazeDisableDuration {
                    ...
                    self.presentEyeTrackingErrorToastIfNeeded()
                } else {
    ...
    override func gazeableHitTest(_ point: CGPoint, with event: UIEyeGazeEvent?) -> UIView? {
    ...
    override func sendEvent(_ originalEvent: UIEvent) {
        ...
        guard let event = originalEvent as? UIEyeGazeEvent,
            let gaze = event.allGazes?.first else {
                if originalEvent.type == .touches {
                    if AppConfig.isEyeTrackingEnabled {
...
@objc extension UIView {
    ...
    fileprivate var gazeGestureRecognizers: [UIEyeGazeRecognizer] {
        return self.gestureRecognizers?.compactMap {
            $0 as? UIEyeGazeRecognizer
        } ?? []
    }
    ...
    func gazeableHitTest(_ point: CGPoint, with event: UIEyeGazeEvent?) -> UIView? {
...
extension UIControl {
    override func gazeBegan(_ gaze: UIEyeGaze, with event: UIEyeGazeEvent?) {
        guard canReceiveGaze else { return }
        self.touchesBegan([gaze], with: nil)
    }

    override func gazeMoved(_ gaze: UIEyeGaze, with event: UIEyeGazeEvent?) {
        guard canReceiveGaze else { return }
        self.touchesMoved(Set([gaze]), with: nil)
    }

    override func gazeEnded(_ gaze: UIEyeGaze, with event: UIEyeGazeEvent?) {
        guard canReceiveGaze else { return }
        self.touchesEnded(Set([gaze]), with: nil)
    }

    override func gazeCancelled(_ gaze: UIEyeGaze, with event: UIEyeGazeEvent?) {
        guard canReceiveGaze else { return }
